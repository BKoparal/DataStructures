...
void Radixsort::countSort(int arr[], int n, int countDizininBoyutu)
{
    ...
    cout<<"           KUMULATIF TOPLAM : "; p.myPrintArray(count,10);
    ...
    cout<<"\ncount dizi : "; p.myPrintArray(count,10);cout<<endl;
    cout<<"\noutput dizi: "; p.myPrintArray(output,n);cout<<endl;
...
void Radixsort::countSort(string arr[], int n, int siralananIndex) {
    cout<<"\nsiralanacak dizi: "; p.myPrintArray(arr,n);
    cout<<"\ncount sort basladi ";
    ...
    for (i = 0; i < n; i++){
        ...
        count[index]++;
        p.myPrintArray(count,26);
        cout<<endl;
    }

    for (i = 1; i < 26; i++)
        count[i] += count[i - 1];
    cout<<"\nAsama 2: KUMULATIF TOPLAM = "; p.myPrintArray(count,26);
    ...
    cout<<"\ncount dizi : "; p.myPrintArray(count,26);cout;
    cout<<"\noutput dizi: "; p.myPrintArray(output,n);cout<<endl;
...
void Radixsort::kovasirala(string D[], int N) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        D[i]= p.mytolower(D[i]); //butun diziyi kucuk harfe cevir
    }
